...
from models.accident import AccidentBase, AccidentCreate, AccidentPublic, AccidentUpdate
from pymongo.collection import Collection
from pymongo import ReturnDocument
from utils.utils import validate_object_id

from schemas.accident import accident_entity_from_db, accident_entities_from_db

ERROR_DETAIL = "Some error occurred: {e}"
NOT_FOUND = "Not found"

async def get_accident_collection(request: Request) -> Collection:
    """Returns the accident collection from MongoDB"""
    return request.app.database["accident"]

AccidentCollection = Annotated[Collection, Depends(get_accident_collection)]

router = APIRouter()

@router.post("/", 
    response_description="Registers a new Accident", 
    status_code=status.HTTP_201_CREATED, response_model=AccidentPublic)
async def create_accident(accident_collection: AccidentCollection, accident: AccidentCreate):
    try:
        ...
        if (created := accident_collection.find_one({"_id": result.inserted_id})) is None:
            raise HTTPException(500, "Failed to create Accident")
            
        return accident_entity_from_db(created)
...
@router.get("/", 
    response_description="Retrieves Accidents", 
    response_model=list[AccidentPublic])
async def read_accidents(
    ...
    cursor = accident_collection.find().skip((page - 1) * limit).limit(limit)
    return accident_entities_from_db(cursor)
